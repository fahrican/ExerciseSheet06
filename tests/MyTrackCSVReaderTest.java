package bwi.prog2B.SS2017.wi16b027_Can.ExerciseSheet06.tests;
...
import bwi.prog2B.SS2017.wi16b027_Can.ExerciseSheet06.util.io.*;
...
public class MyTrackCSVReaderTest {
  ...
  public void getExisting() throws FileNotFoundException {
	  BufferedReader br;
		...
		assertEquals(newTrack.getTitle(), got.getTitle(), "title should be the same");
		assertEquals(newTrack.getWriter().getName(), got.getWriter().getName(), "Writer name should be the same");
		assertEquals(newTrack.getPerformer().getName(), got.getPerformer().getName(), "Performer name should be the same");
		assertEquals(newTrack.getDuration(), got.getDuration(), "duration should be the same");
		assertEquals(newTrack.getYear(), got.getYear(), "year should be the same");
		...
		ByteArrayOutputStream sysOutContent = new ByteArrayOutputStream();
		 ...
		 try{
				// reset stdin in BinaryStdIn through reflection
				  java.lang.reflect.Field out = TextIO.class.getDeclaredField("out");
